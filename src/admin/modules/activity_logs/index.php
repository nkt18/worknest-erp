...
apiFetch(
`<?= htmlspecialchars(appRoute('api.activity_logs')) ?>?page=${page}&search=${search}&user_id=${user}`
)
...
let rows="";
let payload=data.data ?? {data: [], totalPages: 1};
...
createPagination(payload.totalPages);
