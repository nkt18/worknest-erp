...
if (isset($_SESSION['user_id'])) {
    ...
    if ($_SESSION['user_role'] === 'admin') {
        header("Location: " . appRoute('admin.dashboard'));
    } else {
        header("Location: " . appRoute('user.dashboard'));
    }
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (!verifyCsrfToken()) {
        ...
        if (empty($email) || empty($password)) {
            ...
            try {
                loginRateLimitGuard($email);
            } catch (RateLimitException $exception) {
                $error = "Too many login attempts. Try again in " . formatRetryAfter($exception->retryAfterSeconds()) . ".";
            }
        }
